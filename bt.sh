...
# Modifications (c) 2020 Jesse Newland
...
bt_start () {
  if [ -z "$BT_DISABLED" -o "$BT_DISABLED" = "0" ]; then
    ...
    echo "$caller $1" > /tmp/bt.$desc_checksum.$timestamp
...
bt_end () {
  if [ -z "$BT_DISABLED" -o "$BT_DISABLED" = "0" ]; then
    ...
    if [ -z "$2" ]; then
        ...
        echo "$(date '+%s%N' -d "$2") $caller $1" >> /tmp/bt.$desc_checksum
    fi
  fi
}
export -f bt_end
